...
center excerpts from full Spotify tracks using MIR features.
...
# MIR extraction for center clips

def extract_mir_center(filepath, cfg, clip_sec=CLIP_SEC):
    """Extract MIR features for the center clip."""
    ...
    import embed_spaces
    h = embed_spaces._extract_mir(wav, sr, cfg)
...
def main():
    ...
    # ---- Extract MIR embeddings (center 90s) ----
    print("\n=== Extracting MIR embeddings (center 90s) ===")
    raw_embeddings = {}
    for i, t in enumerate(all_tracks):
        fp = t["filepath"]
        print(f"  [{i+1}/{len(all_tracks)}] {t['basename']}...", end="", flush=True)
        emb = extract_mir_center(fp, cfg, clip_sec=CLIP_SEC)
    ...
    reducer = preprocess.PCAReducer(
        dim=cfg["spaces"]["mir"].get("pca_dim", cfg["common"]["pca_dim"]),
    ...
    for i, t in enumerate(all_tracks):
        ...
        x_pca = preprocess.prepare_track(
            x_raw, cfg["spaces"]["mir"], cfg["common"], reducer)
    ...
    cols = ["basename", "genre", "duration_sec", "max_persistence",
            ...
            "p_value", "significant"]
    df[cols].to_csv(os.path.join(out_dir_tables, "loop_spotify_mir.csv"), index=False)
    print(f"\n→ {os.path.join(out_dir_tables, 'loop_spotify_mir.csv')}")
    ...
    if best_sig is not None:
        ...
        plot_track_panel(fp, wav_viz, sr, seconds, cocycle, sub_indices,
                          ...
                          os.path.join(out_dir_figs, "loop_spotify_significant_mir.png"))
        print(f"\n→ {os.path.join(out_dir_figs, 'loop_spotify_significant_mir.png')}")
    ...
    if best_nonsig is not None:
        ...
        plot_track_panel(fp, wav_viz, sr, seconds, cocycle, sub_indices,
                          ...
                          os.path.join(out_dir_figs, "loop_spotify_nonsignificant_mir.png"))
        print(f"→ {os.path.join(out_dir_figs, 'loop_spotify_nonsignificant_mir.png')}")
    ...
    if data_box:
        ...
        ax.set_title(f"Spotify Loop Interpretation (MIR) — Aggregate "
                      f"({n_significant}/{n_informative} significant, "
                      f"n={n_total} tracks)")
        ax.grid(axis="y", alpha=0.3)
        fig_agg.tight_layout()
        fig_agg.savefig(os.path.join(out_dir_figs, "loop_spotify_aggregate_mir.png"),
                         bbox_inches="tight", dpi=150)
        print(f"→ {os.path.join(out_dir_figs, 'loop_spotify_aggregate_mir.png')}")
    plt.close(fig_agg)

    # ---- Save text report ----
    report_path = os.path.join(out_dir_tables, "loop_spotify_report_mir.txt")
    with open(report_path, "w") as f:
        f.write("=" * 70 + "\n")
        f.write("H1 LOOP INTERPRETATION — SPOTIFY FULL TRACKS (90s center, MIR)\n")
        f.write("=" * 70 + "\n\n")
