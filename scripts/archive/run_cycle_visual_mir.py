...
PLAYER_OUT_DIR = "results/figures/player_mir"
...
def main():
    ...
    REDUCER_PATH = "cache/pca_reducer_mir_spotify90.joblib"
    if not os.path.exists(REDUCER_PATH):
        print(f"ERROR: {REDUCER_PATH} not found.")
        sys.exit(1)
    reducer = preprocess.PCAReducer.load(REDUCER_PATH)

    csv_path = os.path.join(cfg["paths"]["results"], "tables", "loop_spotify_mir.csv")
    ...
    for idx, track_base in enumerate(all_tracks):
        ...
        x_pca = preprocess.prepare_track(emb, cfg["spaces"]["mir"], cfg["common"], reducer)
