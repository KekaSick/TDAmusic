...
distances.py
------------
Diagram distances, genre gaps, and Mantel tests.
"""
...
def sampled_distances(diagrams: list[np.ndarray], labels: np.ndarray, n_within: int = 2000, n_between: int = 2000, seed: int = 42):
    """Sample within/between diagram pairs to avoid full O(N^2) computation."""
    from persim import wasserstein
    from tqdm import tqdm
...
def within_between(D: np.ndarray = None, labels: np.ndarray = None, w_arr: np.ndarray = None, b_arr: np.ndarray = None):
    """Return within mean, between mean, and between-within gap."""
...
def bootstrap_gap(D=None, labels=None, w_arr=None, b_arr=None, n_boot=1000, rng=None):
    """Bootstrap the between-within gap."""
...
def bootstrap_gap_by_track(D: np.ndarray, labels: np.ndarray,
    ...
    """Track-level bootstrap for gap = mean(between) - mean(within)."""
    rng = rng or np.random.default_rng(0)
    labels = np.asarray(labels)
    N = D.shape[0]
...
def mantel_test(D1: np.ndarray, D2: np.ndarray,
    ...
    """Mantel test with synchronous row/column permutations."""
    rng = rng or np.random.default_rng(0)
    N = D1.shape[0]
    iu = np.triu_indices(N, k=1)
...
def mantel_bootstrap_ci(D1: np.ndarray, D2: np.ndarray,
    ...
    """Track-level bootstrap CI for the Mantel correlation."""
    rng = rng or np.random.default_rng(0)
    N = D1.shape[0]
...
def mantel(D1: np.ndarray, D2: np.ndarray) -> float:
    """Backward-compatible Pearson Mantel r."""
    return mantel_test(D1, D2)["r"]


if __name__ == "__main__":
    print("=== Inline test: bootstrap_gap_by_track ===")
    ...
    print(f"  boot mean    = {gaps.mean():.4f}  (can differ from gap_observed)")

    assert gap_obs > 0, f"gap should be > 0, got {gap_obs}"
    assert ci[0] > 0, f"CI lower bound should be > 0, got {ci[0]}"
    print("\nbootstrap_gap_by_track: gap > 0 and CI excludes 0")
